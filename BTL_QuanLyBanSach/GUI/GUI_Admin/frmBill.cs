	...
	public partial class frmBill : Form
	{
		...
		private void frmBill_Load(object sender, EventArgs e)
		{
			...
			for (int i = 0; i < dt.Rows.Count; i++)
			{
				...
				button.DisabledState.FillColor2 = Color.FromArgb(169, 169, 169);
				button.FillColor = Color.FromArgb(0, 0, 192);
				button.FillColor2 = SystemColors.ActiveCaption;
		...
		private void btn_BillID_click(object sender, EventArgs e)
		{
			...
			data = dataPro.DataRead("Select BookID, Price, Amount, Discount, TotalCost from BillDetail where BillID = '" + button.Text + "'");
		...
		{
			DataTable data = new DataTable();
			if (BTL.DataPro.Checked.Check(txtSearchBillID.Text))
			{
				MessageBox.Show(
			...
			DateBill.Text = data.Rows[0][1].ToString();
			txtFullName.Text = data.Rows[0][3].ToString();
			data = dataPro.DataRead("Select * from BillDetail where BillID = '" + txtSearchBillID.Text + "'");
			dataViewDetailBill.DataSource = data;
		}
		...
		private void btnPrint_Click(object sender, EventArgs e)
		{
			...
			printBill.Range["A1"].Value = "NTN Bookstore Management";
